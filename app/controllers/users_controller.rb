class UsersController < ApplicationController
  ...
  def update
    if params[:user][:name].present?
      status =  current_user.update_attributes update_user_params
    else
      status =  current_user.update_attributes update_avatar_params
    end
    if status
      add_message_flash :success, t(:updated)
    else
      add_message_flash :error, current_user.errors.full_messages
    end
